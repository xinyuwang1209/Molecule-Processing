...
class CharRNN(nn.Module):
    def __init__(self, vocabulary, config,batch_first=False):
    ...
    def sample(self, n_batch, max_length=100):
        with torch.no_grad():
            ...
            for i in range(1, max_length + 1):
                print(starts)
                output, hiddens = self.forward(starts, hiddens)
    ...
    def configure_optimizers(self, train_config):
        optimizer = torch.optim.AdamW((p for p in self.parameters() if p.requires_grad), lr=train_config.learning_rate)
        return optimizer
